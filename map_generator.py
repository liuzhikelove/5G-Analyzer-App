# ===== File: map_generator.py (最终自洽版 - 内置坐标转换) =====
...
import math # 导入数学库用于坐标转换

# --- [核心修改] 内置坐标转换算法，不再依赖 'coord_convert' ---
...
def create_baidu_map(df_4g, df_5g, results_df, baidu_ak):
    ...
    bmap = (BMap(init_opts=opts.InitOpts(width="100%", height="600px")).add_schema(baidu_ak=baidu_ak, center=[center_lon, center_lat], zoom=14, is_roam=True).add_control_panel(map_type_control_opts=opts.MapTypeControlOpts(type_=BMapType.MAPTYPE_CONTROL_HYBRID)))
