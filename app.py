...
# 优化：仅加载前1000行用于预览，避免大文件内存问题
if uploaded_4g_file and st.session_state.df_4g_preview is None:
    try:
        st.session_state.df_4g_preview = pd.read_excel(uploaded_4g_file, nrows=1000)
    except Exception as e:
...
if uploaded_5g_file and st.session_state.df_5g_preview is None:
    try:
        st.session_state.df_5g_preview = pd.read_excel(uploaded_5g_file, nrows=1000)
    except Exception as e:
        st.error(f"读取5G文件预览时出错：{e}")
        st.session_state.df_5g_preview = None

with preview_4g_placeholder.container(): display_paginated_dataframe(st.session_state.df_4g_preview, "4G数据预览 (前1000行)")
with preview_5g_placeholder.container(): display_paginated_dataframe(st.session_state.df_5g_preview, "5G数据预览 (前1000行)")
