...
def create_map(df_4g, df_5g, results_df):
    ...
    initial_view_state = pdk.ViewState(
        latitude=df_4g_vis['纬度'].mean(),
        longitude=df_4g_vis['经度'].mean(),
        zoom=13,
        pitch=50
    )
    ...
    if not df_5g_vis.empty:
        ...
        layers_to_render.insert(0, heatmap_layer_5g) # 将热力图放在扇区下面
    else:
        st.info("没有有效的5G数据可用于生成热力图。")
...
if st.sidebar.button("🚀 开始分析", type="primary"):
    if st.session_state.df_4g is not None and st.session_state.df_5g is not None:
        df_4g = st.session_state.df_4g.copy() # 使用副本以防修改原始数据
